...
Factory::Factory(
    ...
    : physicsComponents(physicsComponents), renderComponents(renderComponents),
...
void Factory::make_obj(glm::vec3 position, glm::vec3 eulers) {
  ...
  preTransform =
      glm::rotate(preTransform, glm::radians(90.0f), {1.0f, 0.0f, 0.0f});
  preTransform =
...
glm::vec3 Factory::read_vec3(std::vector<std::string> words,
  ...
  return glm::vec3(preTransform * glm::vec4(std::stof(words[1]),
                                            std::stof(words[2]),
                                            std::stof(words[3]), w));
}
...
void Factory::read_corner(std::string description, std::vector<glm::vec3> &v,
                          std::vector<glm::vec2> &vt,
                          std::vector<glm::vec3> &vn,
                          std::vector<float> &vertices) {
